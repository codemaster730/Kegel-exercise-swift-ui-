        ...
        ZStack(alignment: .topLeading){
            ForEach((0...4), id: \.self) { height in
                HStack(alignment: .center){
                    Text("\(self.getYLegendSafe(height: height), specifier: "%.0fm")").offset(x: 0, y: self.getYposition(height: height) )
                        .foregroundColor(Colors.LegendText)
                        .font(.caption)
                    self.line(atHeight: self.getYLegendSafe(height: height), width: self.frame.width)
                        .stroke(self.colorScheme == .dark ? Colors.LegendDarkColor : Colors.LegendColor, style: StrokeStyle(lineWidth: 1.5, lineCap: .round))
            ...
            HStack {
                Spacer()
                ForEach(data.onlyLabels(), id: \.self) { label in
                    Text("\(Int(label) ?? 0, specifier: "%02d")")
                        .foregroundColor(Colors.LegendText)
                        .font(.caption)
                    Spacer()
                }
            }
            .offset(x: 30, y: frame.size.height-padding + 20)
        }
    }
